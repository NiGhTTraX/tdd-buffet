...
If the `--coverage` flag is set then the tests will generate coverage reports at the location specified by `coverageDirectory` from the Jest config. Moreover, the GUI tests will collect **coverage from within the browser**. This requires your code to be instrumented with [istanbul](https://github.com/istanbuljs/babel-plugin-istanbul) and create the coverage report on `window.__coverage__`. That report will be collected at the end of each test and written at `tests/gui/results/coverage/${browser}_${fullTestName}.json`. You can then merge these files yourself with the main report generated by Jest.
